...
```
Create a WordPress custom REST API endpoint to <b><i>functions.php</i></b> file. The example presented below shows how to create a custom route with its own endpoint. The created endpoint has ```GET``` method that returns all data from a WordPress data table.
