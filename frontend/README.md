...
To generate physical build files that you have to connect to your WordPress theme, you need to run the command below. By running this command Vue generates inside the Vue project dist folder that will include the physical files.
...
To let WordPress know about the Vue project, you have to enqueue the Vue bundle in WordPress. Add physical files generated by Vue to functions.php file according the instruction below. The physical files are app.css, chunk-vendors.js and app.js and they can be found from dist folder in the Vue project.
...
Finally, to load the Vue project in WordPress, you need to tell Vue where to mount itself. This is done by specifying the WordPress DOM element in main.js file of your Vue project. The following example allows to use reusable Vue components in DOM elements that id is app.
...
```
Then, add store to main.js file according to the following example.
...
```
Vuex generates to src folder a new folder, which name is store, with index.js file. The content of index.js file looks similiar to the example presented below.
